...
class MangaReaderSpider(CliSpider):
        ...
        for chapter in chapters:
            ...
            if found or oid == start_chapter:
                ...
                yield scrapy.Request(
                    ...
                    meta={"number": num, "name": series_name_clean},
                )
        if not found:
    ...
    def parse(self, response):
        if "/chapter/" not in response.url:
            ...
            series_id, chapter_id = re.match(
                r"https://mangarock.com/manga/(.*)/chapter/(.*)/?", response.url
            ).groups()

            series_url = self.create_url_series(series_id)
            yield scrapy.Request(
                url=series_url, callback=self.parse_series, meta={"start": chapter_id}
            )
