...
class MangaHereSpider(CliSpider):
    ...
    def parse_image(self, response):
        ...
        url_data = self.extract_script_from_html("".join(script_urls), 'var _load_pages')
        ...
        file_path = "/".join([self.name, title, str(chapter).zfill(4), number.zfill(4) + ".jpg"])
        ...
        if next_url and chapter_id in next_url:
            yield scrapy.Request(self.base_site + next_url, callback=self.parse_image, meta=response.meta)
    ...
    def parse(self, response):
        ...
        api_url = response.xpath('//script[contains(@src, "/book-list/")]/@src').extract()
        meta = {'current_id': current_chapter,
                'title': title}
